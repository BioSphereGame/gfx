...
impl Text {
    pub fn new(pos_y: u16, pos_x: u16, size: u16, line_space: u8, char_space: u8, color: u32, font: FontData, text: String, screen_size_y: usize, screen_size_x: usize) -> Text {
        Self {
            text,
            pos_y,
            pos_x,
    ...
    pub fn render_draw(&mut self, screen: &mut crate::Screen) {
        self.render();
        self.draw(screen);
    }
...
//     let mut ascii_escape_counter = 0;
//     for line in self.text.lines() {
//         let glyphs: Vec<PositionedGlyph> = font.layout(line, scale, point(self.pos_x as f32, pos_y_mut as f32 + v_metrics.ascent)).collect();
...
//                     ASCII_RESET => {color = self.color; ascii_escape_delay = ASCII_COLOR_LENGTH; ascii_escape_counter = ASCII_COLOR_LENGTH},
//                     ASCII_BLACK => {color = 0xFF_000000; ascii_escape_delay = ASCII_COLOR_LENGTH; ascii_escape_counter = ASCII_COLOR_LENGTH},
//                     ASCII_RED => {color = 0xFF_FF0000; ascii_escape_delay = ASCII_COLOR_LENGTH; ascii_escape_counter = ASCII_COLOR_LENGTH},
//                     ASCII_GREEN => {color = 0xFF_00FF00; ascii_escape_delay = ASCII_COLOR_LENGTH; ascii_escape_counter = ASCII_COLOR_LENGTH},
//                     ASCII_YELLOW => {color = 0xFF_FFFF00; ascii_escape_delay = ASCII_COLOR_LENGTH; ascii_escape_counter = ASCII_COLOR_LENGTH},
//                     ASCII_BLUE => {color = 0xFF_0000FF; ascii_escape_delay = ASCII_COLOR_LENGTH; ascii_escape_counter = ASCII_COLOR_LENGTH},
//                     ASCII_MAGENTA => {color = 0xFF_FF00FF; ascii_escape_delay = ASCII_COLOR_LENGTH; ascii_escape_counter = ASCII_COLOR_LENGTH},
//                     ASCII_CYAN => {color = 0xFF_00FFFF; ascii_escape_delay = ASCII_COLOR_LENGTH; ascii_escape_counter = ASCII_COLOR_LENGTH},
//                     &_ => {},
//                 }
//             }
...
//                 if let Some(bounding_box) = glyph.pixel_bounding_box() {
//                     glyph.draw(|x, y, v| {
//                         let loc_x = x + bounding_box.min.x as u32 - (self.char_space as u32);
//                         let loc_y = y + bounding_box.min.y as u32 + pos_y_mut as u32;
//                         if loc_x >= screen.size.1 as u32 || loc_y >= screen.size.0 as u32 {
//                             return;
//                         }
//                         let index = loc_y * screen.size.1 as u32 + loc_x; 
//                         if v > 0.01 {
